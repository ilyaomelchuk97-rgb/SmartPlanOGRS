...
## 🖥️ Локальная ДЕМО-версия (без интернета и без сервера)

Если нужно показать или поработать с приложением **без интернета и без Render** — есть полностью автономный вариант.

### Файлы

| Файл | Назначение |
|---|---|
| `index_demo.html` | Главная страница демо-режима |
| `root_index/_demo_api_stub.js` | Заглушка вместо `api_client.js` (все сетевые вызовы → «нет сервера») |
| `install_demo_shortcut.bat` | Установщик ярлыка для **Windows** |
| `install_demo_shortcut.sh` | Установщик ярлыка для **macOS** и **Linux** |

### Чем отличается от обычной версии

| | `index.html` (online) | `index_demo.html` (демо) |
|---|---|---|
| Сервер Render | ✅ нужен | ❌ не нужен |
| Синхронизация между браузерами | ✅ да, polling `/api/sync` каждую секунду | ❌ нет (только локально) |
| Хранение данных | localStorage + PostgreSQL | **только localStorage** |
| Карта погоды Open-Meteo | ✅ да | ✅ если есть интернет (через fetch) |
| Service Worker (PWA) | ✅ зарегистрирован | ❌ отключён |
| Стандартные пользователи | с сервера | из локального seed (`admin`, `seogs`, `master`, `slesar`) |
| Build-маркер | 22.09-27 · Render.com | 22.09-27 · Демо · без Render |

### Установка ярлыка (Windows)

1. Скопируйте всю папку проекта (с `index_demo.html`, `root_index/`, `install_demo_shortcut.bat`) на компьютер, где нет интернета.
2. Запустите `install_demo_shortcut.bat` (двойной клик).
3. На рабочем столе появится ярлык **«SmartPlan Демо»**.
4. Двойной клик по ярлыку откроет `index_demo.html` в браузере (по умолчанию Explorer → открывается в Edge/Chrome).
5. Вход: **admin / admin123**, **master / master123**, **slesar / slesar123**.

### Установка ярлыка (macOS / Linux)

1. Скопируйте папку проекта на компьютер.
2. В терминале:
   ```bash
   cd /путь/к/проекту
   ./install_demo_shortcut.sh
   ```
3. На рабочем столе появится **SmartPlan Demo.app** (macOS) или **SmartPlan Демо.desktop** (Linux).

### Без установщика

Можно просто открыть `index_demo.html` двойным кликом из папки проекта.

### Что работает офлайн

✅ Все экраны: дашборд, календарь, графики, карта маршрутов, карта объектов, разрешения, отчёты, работники, справочники, пользователи, журнал, логи ошибок, тест погодный.  
✅ Создание / редактирование / удаление задач, работников, объектов, графиков, пользователей.  
✅ Экспорт в Excel (через локальный xlsx.full.min.js).  
✅ Drag-and-drop в календаре.

### Что НЕ работает офлайн

❌ Синхронизация между браузерами / коллегами.  
❌ Аутентификация через Render.  
❌ Карта погоды Open-Meteo (если нет интернета).

### Перенос данных между демо и сервером

Данные лежат в localStorage браузера. Чтобы перенести в онлайн-версию:

1. В демо: откройте DevTools → Application → Local Storage → `https://ВАШ_СЕРВЕР/root_index/`
2. Скопируйте JSON из ключей `smartplan_users_db`, `smartplan_objects_db`, `smartplan_tasks_db`, `smartplan_workers_db`, `smartplan_work_catalog`, `smartplan_areas_db`, `smartplan_graphs`
3. В онлайн-версии вставьте в те же ключи через DevTools
4. Перезагрузите страницу

> ⚠ Данные демо-режима хранятся **только в одном браузере**. Если почистить кэш или переустановить браузер — они пропадут.