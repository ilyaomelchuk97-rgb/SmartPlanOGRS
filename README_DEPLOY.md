...
После успешного деплоя (статус **Live**):

1. На странице Web Service → **Shell** (правый верхний угол)
2. Выполните:

```bash
cd /app/server && node migrations/run.js
```
...
✅ Схема БД готова
  ...
  👤 master
  👤 slesar
✅ Стандартные пользователи созданы/обновлены
```

4. Если ошибка `ECONNREFUSED` — БД ещё не подключилась. Подождите 30 сек и повторите.
...
| **Неверный логин / пароль** | Шаг 4 не выполнен (пользователи не созданы) |
